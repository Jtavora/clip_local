...
app = modal.App("minilm-text-embeddings-api")
...
@app.cls(
    ...
    secrets=[modal.Secret.from_name("clip-api-secret")],  # reaproveita: EMBEDDINGS_API_KEY e (opcional) HF_TOKEN
)
...
class MiniLMAPI:
    ...
    def load(self) -> None:
        ...
        # Env vars
        self.MODEL_ID = os.getenv("TEXT_MODEL_ID", "sentence-transformers/all-MiniLM-L6-v2")
        self.NORMALIZE = os.getenv("EMBEDDING_L2_NORMALIZE", "true").lower() == "true"
        self.USE_AMP = os.getenv("USE_AMP_FP16", "true").lower() == "true"

        # vindo do Secret (se existir)
        self.API_KEY = os.getenv("EMBEDDINGS_API_KEY")
        self.HF_TOKEN = os.getenv("HF_TOKEN")  # opcional, se o modelo exigir auth

        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.use_amp = (self.device == "cuda") and self.USE_AMP

        # Load 1x
        # (AutoTokenizer/AutoModel + mean pooling com attention mask)
        ...
        self.EMBED_DIM = int(getattr(self.model.config, "hidden_size", 384))

        # Limites opcionais (pra evitar payloads absurdos)
    ...
    def web(self):
        ...
        api = FastAPI(title="MiniLM Text Embeddings API (Modal)", version="1.0.0")
