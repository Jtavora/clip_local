...
@app.cls(
    ...
    secrets=[modal.Secret.from_name("clip-api-secret")],  # EMBEDDINGS_API_KEY + HF_TOKEN (opcional)
)
...
class ABNTAPI:
    ...
    def load(self) -> None:
        ...
        dtype_name = os.getenv("ABNT_TORCH_DTYPE", "auto").strip().lower()
        dtype_map = {
            "bf16": torch.bfloat16,
        ...
        self.TORCH_DTYPE = dtype_map.get(dtype_name, "auto")
    ...
    def _extract_payload_from_generated(self, generated_text: str) -> dict:
        parsed_text = ""
        if hasattr(self.processor, "parse_response"):
            try:
                parsed = self.processor.parse_response(generated_text)
    ...
    def _extract_from_text(self, text: str, source_name: str) -> tuple[ReferenciaLivroABNT, dict]:
        ...
        generated = self.processor.decode(outputs[0][input_len:], skip_special_tokens=False)

        payload = self._extract_payload_from_generated(generated)
    ...
    def web(self):
        ...
        api = FastAPI(title="ABNT Reference API (Modal - Gemma 4 E4B)", version="2.0.0")
        ...
        async def extract_file(
            ...
            if not extracted.strip():
                raise HTTPException(
                    status_code=400,
                    detail="não foi possível extrair texto útil do arquivo",
                )
