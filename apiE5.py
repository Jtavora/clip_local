...
app = modal.App("e5-text-embeddings-api")
...
class E5API:
    ...
    def load(self) -> None:
        ...
        self.MODEL_ID = os.getenv("TEXT_MODEL_ID", "intfloat/multilingual-e5-large")
        self.NORMALIZE = os.getenv("EMBEDDING_L2_NORMALIZE", "true").lower() == "true"
        self.USE_AMP = os.getenv("USE_AMP_FP16", "true").lower() == "true"
        self.MAX_LENGTH = int(os.getenv("E5_MAX_LENGTH", "512"))

        default_prefix = os.getenv("E5_DEFAULT_PREFIX", "query").strip().lower()
        self.DEFAULT_PREFIX = default_prefix if default_prefix in ("query", "passage") else "query"
    ...
    def _format_e5_text(self, text: str) -> str:
        t = text.strip()
        tl = t.lower()
        if tl.startswith("query:") or tl.startswith("passage:"):
            return t
        return f"{self.DEFAULT_PREFIX}: {t}"

    def _average_pooling(self, last_hidden_states, attention_mask):
        import torch

        masked = last_hidden_states.masked_fill(~attention_mask[..., None].bool(), 0.0)
        denom = torch.clamp(attention_mask.sum(dim=1)[..., None], min=1e-9)
        return masked.sum(dim=1) / denom

    def _embed_texts(self, texts: list[str]):
        import torch

        formatted = [self._format_e5_text(t) for t in texts]
        ...
        feats = self._average_pooling(out.last_hidden_state, encoded["attention_mask"])
    ...
    def web(self):
        ...
        api = FastAPI(title="Multilingual E5 Text Embeddings API (Modal)", version="1.0.0")
